...
double tsp_mst(point *V, int n, int *Q, graph T) {
  ...
  //chaque aretes reliant chaque sommets du graphe
  int m = n * (n-1)/2;
  // E = tableau de toutes les arêtes définies à partir des n points de V
  edge *E = (edge *) malloc(m * sizeof(edge)); // à remplacer par un malloc(...)

  //Initialize the egde list with every possibilities
  int i =0;
  for(int u = 0; u < n; u++){
    for(int v = u+1; v < n; v++){
  ...
  for(int i = 0, nb_union = 0; nb_union < n-1; i++){
    edge e = E[i];

    int parent_left = Find(e.u, parent);
    int parent_right = Find(e.v, parent);


    //printf("parentleft = %d, parentright = %d\n", parent_left, parent_right);
    if(parent_left != parent_right){
      Union(parent_left, parent_right, parent, height);
      addEdge(T, e.u, e.v);
  ...
  dfs(T, 1, Q, -1);      // calcule Q grâce au DFS à partir du sommet 0 de T
